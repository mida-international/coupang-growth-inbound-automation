...
export async function configureNegativeStockSearch(
  ...
  const startInput = await firstVisibleLocator(frame, [
    ...STOCK_RANGE_START_SELECTORS,
  ]);
  ...
  const endInput = await firstVisibleLocator(frame, [
    ...STOCK_RANGE_END_SELECTORS,
  ]);
...
export async function clickInventorySearch(frame: Frame): Promise<void> {
  const searchButton = await firstVisibleLocator(frame, [
...
export async function clickInventoryExcelDownload(
  ...
  const excelButton = await firstVisibleLocator(frame, [
