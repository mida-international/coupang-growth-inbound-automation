...
type WarehouseInboundRecordHistorySectionProps = {
  data: ListWarehouseInboundDeliverablesResult;
};
...
function buildHistoryPageHref(page: number, pageSize: number): string {
  ...
  return query.length > 0
    ? `/data/dashboard/warehouse-inbound?${query}`
    : "/data/dashboard/warehouse-inbound";
}
...
function HistoryRow({
  ...
  row: WarehouseInboundDeliverableListItem;
  onDeleted: () => void;
}) {
  ...
  async function handleDownload() {
    ...
    try {
      const response = await fetch(
        `/api/warehouse-inbound-deliverables/${row.id}/download`,
      );
  ...
  async function handleDelete() {
    ...
    const result = await apiDelete<void>(
      `/api/warehouse-inbound-deliverables/${row.id}`,
    );
  ...
  return (
    <>
      <TableRow>
        ...
        <TableCell>{row.recordDate}</TableCell>
        <TableCell>{formatRotationLabel(row.rotationCount)}</TableCell>
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {row.items.map((item, index) => (
                        <TableRow
                          key={`${row.id}-${item.productBarcode ?? "row"}-${index}`}
                        >
                          <TableCell>{item.recordDate}</TableCell>
                          <TableCell>{formatCell(item.location)}</TableCell>
                          <TableCell>
                            {formatCell(item.registeredProductName)}
                          </TableCell>
                          <TableCell>{formatCell(item.optionName)}</TableCell>
                          <TableCell>
                            {formatCell(item.productBarcode)}
                          </TableCell>
                          <TableCell className="text-right">
...
export function WarehouseInboundRecordHistorySection({
  data,
}: WarehouseInboundRecordHistorySectionProps) {
  ...
  return (
    <DeliverablesSection
      title="입고리스트 기록 목록"
      description="기록하기로 저장된 창고 전송용 입고리스트 엑셀과 B~G열 데이터를 조회합니다."
      variant="plain"
    >
      {data.rowCount === 0 ? (
        <p className="text-sm text-muted-foreground">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex flex-col items-end gap-2">
              ...
              <ListExcelDownloadButton
                downloadHref="/api/downloads/warehouse-inbound-history"
