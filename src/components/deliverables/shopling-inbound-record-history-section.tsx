...
type ShoplingInboundRecordHistorySectionProps = {
  data: ListShoplingInboundDeliverablesResult;
};
...
function buildHistoryPageHref(page: number, pageSize: number): string {
  ...
  return query.length > 0
    ? `/data/dashboard/shopling-inbound?${query}`
    : "/data/dashboard/shopling-inbound";
}
...
function HistoryRow({
  ...
  row: ShoplingInboundDeliverableListItem;
  onDeleted: () => void;
}) {
  ...
  async function handleDownload() {
    ...
    try {
      const response = await fetch(
        `/api/shopling-inbound-deliverables/${row.id}/download`,
      );
  ...
  async function handleDelete() {
    const confirmed = window.confirm(
      `"${row.outputFileName}" 입고 기록과 저장된 엑셀을 삭제할까요?`,
    );
    ...
    const result = await apiDelete<void>(
      `/api/shopling-inbound-deliverables/${row.id}`,
    );
      ...
      <TableRow>
        <TableCell>
          ...
          <TableCell className="text-right">
            {row.barcodeCount.toLocaleString()}
          </TableCell>
          <TableCell className="text-right">
            ...
            <div className="border-t border-border px-4 py-3">
              ...
              <div className="overflow-x-auto rounded-md border border-border bg-background">
                <Table>
                  ...
                  <TableBody>
                    {row.items.map((item) => (
                      <TableRow key={`${row.id}-${item.barcode}`}>
...
export function ShoplingInboundRecordHistorySection({
  data,
}: ShoplingInboundRecordHistorySectionProps) {
  ...
  return (
    <DeliverablesSection
      title="입고 기록 목록"
      description="입고 기록하기로 저장된 WMS 엑셀과 바코드별 수량을 조회합니다."
      variant="plain"
    >
      {data.rowCount === 0 ? (
              ...
              <Table>
                <TableHeader className="bg-muted/40">
                  <TableRow className="hover:bg-transparent">
                    ...
                    <TableHead className="text-right">바코드 수</TableHead>
