...
import type {
  ShoplingInboundDeliverableFile,
  ShoplingInboundDeliverableServiceResult,
} from "@/services/deliverables/types";

export async function getShoplingInboundDeliverableFile(
  id: string,
): Promise<ShoplingInboundDeliverableServiceResult<ShoplingInboundDeliverableFile>> {
  if (!id.trim()) {
