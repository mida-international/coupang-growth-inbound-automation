...
import type { WarehouseInboundDeliverableServiceResult } from "@/services/deliverables/types";

export async function deleteWarehouseInboundDeliverable(
  id: string,
): Promise<WarehouseInboundDeliverableServiceResult<void>> {
  if (!id.trim()) {
  ...
  try {
    await prisma.warehouseInboundDeliverable.delete({ where: { id } });
  } catch {
